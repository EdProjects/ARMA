_weaponArray = []; //classname, description, type, [magazine class, magazine description, magazine icon], picture
...
for[{_i = 0},{_i < count(_weaponsConfig)},{_i = _i + 1}] do
{
	...
	if(isClass _currentWeapon) then {
		if(getNumber(_currentWeapon >> "scope") == 2) then	{
			...
			_weaponArray = _weaponArray + [[_className, _description, _type, _magazines, _picture]];
		};
	};
};
